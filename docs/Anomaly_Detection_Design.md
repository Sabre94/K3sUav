...
**异常检测器 (Anomaly Detector)** 是部署在**云端调度器**上的AI模块，用于检测无人机节点的异常状态，确保调度决策的安全性和可靠性。

### 1.1 云端部署的优势

| 优势 | 说明 |
|------|------|
| **全局视角** | 云端可同时分析所有UAV的数据，发现集群级异常 |
| **计算资源充足** | 可运行复杂的ML模型（如Isolation Forest） |
| **历史数据丰富** | 结合状态预测器的历史存储，进行时序分析 |
| **边缘无负担** | UAV不需要运行检测算法，节省边缘计算资源 |
